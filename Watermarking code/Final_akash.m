...
while chos~=possibility,
    chos=menu('Chaos based watermarking','select the cover image','watermark encryption','seelect encrypted watermarked image','show extracted image','show  extracted watermark','show decrypted watermark','PSNR and MSE');
    if chos==1
...
imshow(bxe)
 secfn = input('Enter File Name for Image + Message:\n','s');
    ...
    if chos==3
   [fname pname]=uigetfile('*.jpg','select the encrypted Watermark');
    imw2=imread(fname);
    imw=rgb2gray(imw2);
